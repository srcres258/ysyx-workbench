...
static inline void difftest_skip_ref() {
  difftest_skip_ref_with_reason(DIFFTEST_SKIP_REASON_NONDETERMINISTIC);
}
...
#else
static inline void difftest_skip_ref_with_reason(DiffTestSkipReason reason) { (void) reason; }
static inline void difftest_skip_ref() {}
...
extern void (*ref_difftest_set_mem_map)(const DiffTestMemRegion *regions, size_t nr_regions);
extern size_t (*ref_difftest_get_mem_map)(DiffTestMemRegion *regions, size_t max_regions);
extern void (*ref_difftest_set_reset_vector)(uint64_t reset_vector);
