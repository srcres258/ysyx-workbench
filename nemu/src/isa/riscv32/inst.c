...
#define SEXT_REG(x) ((word_t) SEXT(x, sizeof(word_t) * 8))
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb      , I, R(rd) = SEXT_REG(Mr(src1 + imm, 1)));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh      , I, R(rd) = SEXT_REG(Mr(src1 + imm, 2)));
