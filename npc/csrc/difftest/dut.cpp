...
static inline SimulatorImpl& sim() { return *getActiveSimulator(); }
...
static void loadRefSymbols(void *dlHandle) {
    ...
    s.ref_difftest_memcpy = (ref_difftest_memcpy_f_t) dlsym(dlHandle, "difftest_memcpy");
    assert(s.ref_difftest_memcpy);

    std::cout << "正在加载 difftest_regcpy ..." << std::endl;
    s.ref_difftest_regcpy = (ref_difftest_regcpy_f_t) dlsym(dlHandle, "difftest_regcpy");
    assert(s.ref_difftest_regcpy);

    std::cout << "正在加载 difftest_exec ..." << std::endl;
    s.ref_difftest_exec = (ref_difftest_exec_f_t) dlsym(dlHandle, "difftest_exec");
    assert(s.ref_difftest_exec);

    std::cout << "正在加载 difftest_raise_intr ..." << std::endl;
    s.ref_difftest_raise_intr = (ref_difftest_raise_intr_f_t) dlsym(dlHandle, "difftest_raise_intr");
    assert(s.ref_difftest_raise_intr);

    std::cout << "正在加载 difftest_set_mem_map ..." << std::endl;
    s.ref_difftest_set_mem_map = (ref_difftest_set_mem_map_f_t) dlsym(dlHandle, "difftest_set_mem_map");
    assert(s.ref_difftest_set_mem_map);

    std::cout << "正在加载 difftest_get_mem_map ..." << std::endl;
    s.ref_difftest_get_mem_map = (ref_difftest_get_mem_map_f_t) dlsym(dlHandle, "difftest_get_mem_map");
    assert(s.ref_difftest_get_mem_map);

    std::cout << "正在加载 difftest_set_reset_vector ..." << std::endl;
    s.ref_difftest_set_reset_vector = (ref_difftest_set_reset_vector_f_t) dlsym(dlHandle, "difftest_set_reset_vector");
    assert(s.ref_difftest_set_reset_vector);

    std::cout << "正在加载 difftest_init ..." << std::endl;
    s.ref_difftest_init = (ref_difftest_init_f_t) dlsym(dlHandle, "difftest_init");
...
void difftest_dut_init(const char *refSoFile, int port) {
    ...
    if (s.ref_difftest_get_mem_map) {
        ...
        size_t mapCount = s.ref_difftest_get_mem_map(debugMap, sizeof(debugMap) / sizeof(debugMap[0]));
    ...
    if (s.mrom_io_base != nullptr) {
        s.ref_difftest_memcpy(MROM_ADDR, s.mrom_io_base, MROM_LEN, DIFFTEST_TO_REF);
    }
    if (s.flash_io_base != nullptr) {
        s.ref_difftest_memcpy(FLASH_ADDR, s.flash_io_base, FLASH_LEN, DIFFTEST_TO_REF);
    }
#endif
