...
`ysyx-workbench` is a wrapper repo for NEMU, AbstractMachine, NPC, ysyxSoC, rt-thread, am-kernels, NVBoard, and fceux-am.

## Before you touch anything

- Run `nix develop` first; it sets `NEMU_HOME`, `AM_HOME`, `NPC_HOME`, `NVBOARD_HOME`, `YSYX_HOME`, and `VERILATOR_HOME`.
- `config.fish` is not a full setup path; ignore it for environment bootstrap.
- `init.sh` is the legacy bootstrap path referenced by `README.md`; it clones subprojects and writes env vars to `~/.bashrc`.
...
- Check `.sisyphus/plans/` before related feature work; the repo already carries task-specific plans.
